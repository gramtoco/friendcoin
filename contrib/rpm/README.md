...
The RPM spec file provided here is for Pejecoin-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://pejecoin.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Pejecoin git repository](https://github.com/PejecoinBlockchain/Pejecoin) but are not part of
...
Currently only `Source10` is used, for the example `pejecoin.conf` file.
...
in the pejecoin git repository. At present only an SVG version of the Pejecoin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the pejecoin
...
needed when 0.12.1 is released, a proper fix is already in the Pejecoin git
master branch.
...
idea to build Pejecoin Core against a static version of that library compiled
...
The `pejecoin.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `pejecoin-server` package. At this point, most RPM based
...
The `pejecoin-server` package should have SELinux support. How to properly do
...
Pejecoin-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/pejecoin/). He has been
packaging Pejecoin for Fedora at least since 2012.
...
4. I renamed the `pejecoin` package that contains the Qt GUI to `pejecoin-core` as
that appears to be how the general population refers to it, in contrast to
`pejecoin-xt` or `pejecoin-classic`. I wanted to make sure the general population
