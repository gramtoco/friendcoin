...
bool GenerateDistributionList(const CRewardSnapshot& p_rewardSnapshot, std::vector<OwnerAndAmount>& vecDistributionList)
{
    ...
    CAmount srcUnitDivisor = COIN;  //  Default to divisor for FRENS
    ...
    if (p_rewardSnapshot.strDistributionAsset != "FRENS") {
        ...
        LogPrint(BCLog::REWARDS, "%s: Distribution is FRENS with divisor %d\n", __func__, srcUnitDivisor);
    }
        ...
        std::string& change_address, uint256& retTxid)
{
    ...
    //  Handle payouts using FRENS differently from those using an asset
    if (p_rewardSnapshot.strDistributionAsset == "FRENS") {
        // Check amount
        ...
        for (int i = start; i < (int)p_pendingPayments.size() && i < stop; i++) {
            ...
            // Parse Fren address (already validated during ownership snapshot creation)
