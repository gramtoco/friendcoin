...
bool GenerateDistributionList(const CRewardSnapshot& p_rewardSnapshot, std::vector<OwnerAndAmount>& vecDistributionList)
{
    ...
    CAmount srcUnitDivisor = COIN;  //  Default to divisor for PEJE
    ...
    if (p_rewardSnapshot.strDistributionAsset != "PEJE") {
        ...
        LogPrint(BCLog::REWARDS, "%s: Distribution is PEJE with divisor %d\n", __func__, srcUnitDivisor);
    }
        ...
        std::string& change_address, uint256& retTxid)
{
    ...
    //  Handle payouts using PEJE differently from those using an asset
    if (p_rewardSnapshot.strDistributionAsset == "PEJE") {
        // Check amount
        ...
        for (int i = start; i < (int)p_pendingPayments.size() && i < stop; i++) {
            ...
            // Parse Pejecoin address (already validated during ownership snapshot creation)
