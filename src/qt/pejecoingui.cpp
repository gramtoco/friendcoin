...
const std::string PejecoinGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
        "macosx"
...
const QString PejecoinGUI::DEFAULT_WALLET = "~Default";
...
static bool ThreadSafeMessageBox(PejecoinGUI *gui, const std::string& message, const std::string& caption, unsigned int style);

PejecoinGUI::PejecoinGUI(const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    enableWallet(false),
...
void PejecoinGUI::createActions()
{
    QFont font = QFont();
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Pejecoin address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and pejecoin: URIs)"));
    ...
    transferAssetAction->setStatusTip(tr("Transfer assets to PEJE addresses"));
...
//    messagingAction->setShortcut(QKeySequence(Qt::ALT + Qt::Key_9));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Pejecoin addresses to prove you own them"));
    verifyMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Pejecoin addresses"));
    ...
    openAction->setStatusTip(tr("Open a pejecoin: URI or payment request"));

    showHelpMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Pejecoin command-line options").arg(tr(PACKAGE_NAME)));
...
void PejecoinGUI::createMenuBar()
{
#ifdef Q_OS_MAC
...
void PejecoinGUI::createToolBars()
{
    if(walletFrame)
    {
        QSettings settings;
        ...
        if(IconsOnly) {
            labelToolbar->setPixmap(QPixmap::fromImage(QImage(":/icons/pejetext")));
        }
        else {
            labelToolbar->setPixmap(QPixmap::fromImage(QImage(":/icons/pejecointext")));
        }
...
QString tbStyleSheet = ".QToolBar {background-color: transparent; border-color: transparent; }  "
        ...
        QLabel* pejecoinLabel = new QLabel("PEJECOIN");
        pejecoinLabel->setAlignment(Qt::AlignCenter);
        pejecoinLabel->setStyleSheet("color: white; font-weight: bold; font-size: 20px;"); // Adjust the font size as needed
        ...
        QVBoxLayout* pejeLabelLayout = new QVBoxLayout(toolbarWidget);
        pejeLabelLayout->addWidget(labelToolbar);
        pejeLabelLayout->addWidget(m_toolbar);
        pejeLabelLayout->addStretch(1);
        pejeLabelLayout->addWidget(pejecoinLabel);
        pejeLabelLayout->addWidget(versionLabel);  // Add the QLabel for version
        pejeLabelLayout->setDirection(QBoxLayout::TopToBottom);
        ...
        labelCurrentMarket->setText(tr("Pejecoin Market Price"));
        ...
        comboPejeUnit = new QComboBox(headerWidget);
        QStringList list;
        for(int unitNum = 0; unitNum < CurrencyUnits::count(); unitNum++) {
            list.append(QString(CurrencyUnits::CurrencyOptions[unitNum].Header));
        }
        comboPejeUnit->addItems(list);
        comboPejeUnit->setFixedHeight(26);
        comboPejeUnit->setContentsMargins(5,0,0,0);
        comboPejeUnit->setStyleSheet(STRING_LABEL_COLOR);
        comboPejeUnit->setFont(currentMarketFont);

        labelVersionUpdate->setText("<a href=\"https://github.com/PejecoinBlockchain/Pejecoin/releases\">New Wallet Version Available</a>");
        ...
        priceLayout->addWidget(comboPejeUnit, 0 , Qt::AlignBottom| Qt::AlignLeft);
        ...
        connect(comboPejeUnit, SIGNAL(activated(int)), this, SLOT(currencySelectionChanged(int)));
        // Create the timer
        ...
        // Get the latest Pejecoin release and let the user know if they are using the latest version
        // Network request code for the header widget
                       ...
                       if (key == "tag_name") {
                           ...
                           if (fNewSoftwareFound) {
                               ...
                               if (GetRandInt(2) == 1) {
                                   ...
                                   if (fRet) {
                                       QString link = "https://github.com/PejecoinBlockchain/Pejecoin/releases";
...
void PejecoinGUI::updateIconsOnlyToolbar(bool IconsOnly)
{
    if(IconsOnly) {
        labelToolbar->setPixmap(QPixmap::fromImage(QImage(":/icons/pejetext")));
        m_toolbar->setMaximumWidth(65);
...
void PejecoinGUI::setClientModel(ClientModel *_clientModel)
{
...
#ifdef ENABLE_WALLET
bool PejecoinGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
bool PejecoinGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void PejecoinGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void PejecoinGUI::setWalletActionsEnabled(bool enabled)
{
...
void PejecoinGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
#ifndef Q_OS_MAC
...
void PejecoinGUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
...
#ifndef Q_OS_MAC
void PejecoinGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void PejecoinGUI::optionsClicked()
{
...
void PejecoinGUI::showDebugWindow()
{
...
void PejecoinGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_CONSOLE);
    showDebugWindow();
}

void PejecoinGUI::showWalletRepair()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_REPAIR);
    showDebugWindow();
}

void PejecoinGUI::showHelpMessageClicked()
{
    helpMessageDialog->show();
}

#ifdef ENABLE_WALLET
void PejecoinGUI::openClicked()
{
...
void PejecoinGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void PejecoinGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void PejecoinGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void PejecoinGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void PejecoinGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void PejecoinGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}

/** PEJE START */
void PejecoinGUI::gotoAssetsPage()
{
    transferAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoAssetsPage();
};

void PejecoinGUI::gotoCreateAssetsPage()
{
    createAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoCreateAssetsPage();
};

void PejecoinGUI::gotoManageAssetsPage()
{
    manageAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoManageAssetsPage();
};

void PejecoinGUI::gotoRestrictedAssetsPage()
{
...
void PejecoinGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        tooltip = tr("%n active connection(s) to Pejecoin network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
...
void PejecoinGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void PejecoinGUI::setNetworkActive(bool networkActive)
{
    updateNetworkState();
}

void PejecoinGUI::updateHeadersSyncProgressLabel()
{
...
void PejecoinGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if (modalOverlay)
    {
        if (header)
...
void PejecoinGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("Pejecoin"); // default title
...
void PejecoinGUI::changeEvent(QEvent *e)
{
...
void PejecoinGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void PejecoinGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void PejecoinGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& assetName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date);
    if (assetName == "PEJE")
        msg += tr("Amount: %1\n").arg(PejecoinUnits::formatWithUnit(unit, amount, true));
    else
        msg += tr("Amount: %1\n").arg(PejecoinUnits::formatWithCustomName(assetName, amount, MAX_ASSET_UNITS, true));
...
void PejecoinGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void PejecoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool PejecoinGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool PejecoinGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void PejecoinGUI::setHDStatus(int hdEnabled)
{
    QString icon = "";
...
void PejecoinGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void PejecoinGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void PejecoinGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void PejecoinGUI::detectShutdown()
{
...
void PejecoinGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void PejecoinGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
void PejecoinGUI::showModalOverlay()
{
...
static bool ThreadSafeMnemonic(PejecoinGUI *gui, unsigned int style)
{
    bool modal = (style & CClientUIInterface::MODAL);
    // The SECURE flag has no effect in the Qt GUI.
    // bool secure = (style & CClientUIInterface::SECURE);
...
void PejecoinGUI::subscribeToCoreSignals()
{
...
void PejecoinGUI::unsubscribeFromCoreSignals()
{
...
void PejecoinGUI::toggleNetworkActive()
{
    if (clientModel) {
...
void PejecoinGUI::handleRestart(QStringList args)
{
    if (!ShutdownRequested())
    ...
    menu(0)
{
    ...
    QList<PejecoinUnits::Unit> units = PejecoinUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const PejecoinUnits::Unit unit : units)
    {
    #ifndef QTversionPreFiveEleven
        max_width = qMax(max_width, fm.horizontalAdvance(PejecoinUnits::name(unit)));
    #else
        max_width = qMax(max_width, fm.width(PejecoinUnits::name(unit)));
    #endif
    }
...
{
    menu = new QMenu(this);
    for (PejecoinUnits::Unit u : PejecoinUnits::availableUnits())
    {
        QAction *menuAction = new QAction(QString(PejecoinUnits::name(u)), this);
...
{
    setText(PejecoinUnits::name(newUnits));
}
...
void PejecoinGUI::currencySelectionChanged(int unitIndex)
{
    if(clientModel && clientModel->getOptionsModel())
    {
...
void PejecoinGUI::onCurrencyChange(int newIndex)
{
    qDebug() << "PejecoinGUI::onPriceUnitChange: " + QString::number(newIndex);
    ...
    this->comboPejeUnit->setCurrentIndex(newIndex);
    this->getPriceInfo();
}

void PejecoinGUI::getPriceInfo()
{
    request->setUrl(QUrl(QString("https://api.xeggex.com/api/v2/ticker/PEJE_%1").arg(this->currentPriceDisplay->Ticker)));
    networkManager->get(*request);
}

#ifdef ENABLE_WALLET
void PejecoinGUI::mnemonic()
{
...
void PejecoinGUI::getLatestVersion()
{
    versionRequest->setUrl(QUrl("https://api.github.com/repos/PejecoinProject/Pejecoin/releases"));
