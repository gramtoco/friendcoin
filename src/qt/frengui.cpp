...
const std::string FrenGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
        "macosx"
...
const QString FrenGUI::DEFAULT_WALLET = "~Default";
...
std::array<CurrencyUnitDetails, 5> CurrencyUnits::CurrencyOptions = { {
    { "BTC",    "FRENSBTC"  , 1,          8},
    { "mBTC",   "FRENSBTC"  , 1000,       5},
    { "µBTC",   "FRENSBTC"  , 1000000,    2},
    { "Satoshi","FRENSBTC"  , 100000000,  0},
    { "USDT",   "FRENSUSDT" , 1,          5}
} };

static bool ThreadSafeMessageBox(FrenGUI *gui, const std::string& message, const std::string& caption, unsigned int style);

FrenGUI::FrenGUI(const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    enableWallet(false),
...
void FrenGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Fren address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and fren: URIs)"));
    ...
    transferAssetAction->setStatusTip(tr("Transfer assets to FRENS addresses"));
...
//    messagingAction->setShortcut(QKeySequence(Qt::ALT + Qt::Key_9));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Fren addresses to prove you own them"));
    verifyMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Fren addresses"));
    ...
    openAction->setStatusTip(tr("Open a fren: URI or payment request"));

    showHelpMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Fren command-line options").arg(tr(PACKAGE_NAME)));
...
void FrenGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        if(IconsOnly) {
            labelToolbar->setPixmap(QPixmap::fromImage(QImage(":/icons/frenstext")));
        }
        else {
            labelToolbar->setPixmap(QPixmap::fromImage(QImage(":/icons/frencointext")));
        }
...
//        m_toolbar->addAction(votingAction);
        ...
        QVBoxLayout* frenLabelLayout = new QVBoxLayout(toolbarWidget);
        frenLabelLayout->addWidget(labelToolbar);
        frenLabelLayout->addWidget(m_toolbar);
        frenLabelLayout->setDirection(QBoxLayout::TopToBottom);
        frenLabelLayout->addStretch(1);
        ...
        labelCurrentMarket->setText(tr("Frencoin 1 Fren = 1 Fren"));
        ...
        labelVersionUpdate->setText("<a href=\"https://github.com/Apushii/Frencoin/releases\">New Wallet Version Available</a>");
        ...
        // Get the latest Frencoin release and let the user know if they are using the latest version
        // Network request code for the header widget
                       ...
                       if (key == "tag_name") {
                           ...
                           if (fNewSoftwareFound) {
                               ...
                               if (GetRandInt(2) == 1) {
                                   ...
                                   if (fRet) {
                                       QString link = "https://github.com/Apushii/Frencoin/releases";
...
void FrenGUI::updateIconsOnlyToolbar(bool IconsOnly)
{
    if(IconsOnly) {
        labelToolbar->setPixmap(QPixmap::fromImage(QImage(":/icons/frenstext")));
        m_toolbar->setMaximumWidth(65);
...
void FrenGUI::setClientModel(ClientModel *_clientModel)
{
...
#ifdef ENABLE_WALLET
bool FrenGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
bool FrenGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void FrenGUI::setWalletActionsEnabled(bool enabled)
{
...
void FrenGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
#ifndef Q_OS_MAC
...
void FrenGUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
...
#ifndef Q_OS_MAC
void FrenGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void FrenGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_CONSOLE);
    showDebugWindow();
}

void FrenGUI::showWalletRepair()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_REPAIR);
    showDebugWindow();
}

void FrenGUI::showHelpMessageClicked()
{
    helpMessageDialog->show();
}

#ifdef ENABLE_WALLET
void FrenGUI::openClicked()
{
...
void FrenGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void FrenGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void FrenGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void FrenGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void FrenGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void FrenGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}

/** FRENS START */
void FrenGUI::gotoAssetsPage()
{
    transferAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoAssetsPage();
};

void FrenGUI::gotoCreateAssetsPage()
{
    createAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoCreateAssetsPage();
};

void FrenGUI::gotoManageAssetsPage()
{
    manageAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoManageAssetsPage();
};

void FrenGUI::gotoRestrictedAssetsPage()
{
...
void FrenGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        tooltip = tr("%n active connection(s) to Fren network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
...
void FrenGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void FrenGUI::setNetworkActive(bool networkActive)
{
    updateNetworkState();
}

void FrenGUI::updateHeadersSyncProgressLabel()
{
...
void FrenGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if (modalOverlay)
    {
        if (header)
...
void FrenGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("Fren"); // default title
...
void FrenGUI::changeEvent(QEvent *e)
{
...
void FrenGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void FrenGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void FrenGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& assetName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date);
    if (assetName == "FRENS")
        msg += tr("Amount: %1\n").arg(FrenUnits::formatWithUnit(unit, amount, true));
    else
        msg += tr("Amount: %1\n").arg(FrenUnits::formatWithCustomName(assetName, amount, MAX_ASSET_UNITS, true));
...
void FrenGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void FrenGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool FrenGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool FrenGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void FrenGUI::setHDStatus(int hdEnabled)
{
    QString icon = "";
...
void FrenGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void FrenGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void FrenGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void FrenGUI::detectShutdown()
{
...
void FrenGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void FrenGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
static bool ThreadSafeMnemonic(FrenGUI *gui, unsigned int style)
{
    bool modal = (style & CClientUIInterface::MODAL);
    // The SECURE flag has no effect in the Qt GUI.
    // bool secure = (style & CClientUIInterface::SECURE);
...
void FrenGUI::subscribeToCoreSignals()
{
...
void FrenGUI::unsubscribeFromCoreSignals()
{
...
void FrenGUI::toggleNetworkActive()
{
    if (clientModel) {
...
void FrenGUI::handleRestart(QStringList args)
{
    if (!ShutdownRequested())
    ...
    menu(0)
{
    ...
    QList<FrenUnits::Unit> units = FrenUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const FrenUnits::Unit unit : units)
    {
    #ifndef QTversionPreFiveEleven
        max_width = qMax(max_width, fm.horizontalAdvance(FrenUnits::name(unit)));
    #else
        max_width = qMax(max_width, fm.width(FrenUnits::name(unit)));
    #endif
    }
...
{
    menu = new QMenu(this);
    for (FrenUnits::Unit u : FrenUnits::availableUnits())
    {
        QAction *menuAction = new QAction(QString(FrenUnits::name(u)), this);
...
{
    setText(FrenUnits::name(newUnits));
}
...
void FrenGUI::currencySelectionChanged(int unitIndex)
{
    if(clientModel && clientModel->getOptionsModel())
    {
...
void FrenGUI::onCurrencyChange(int newIndex)
{
    qDebug() << "FrenGUI::onPriceUnitChange: " + QString::number(newIndex);
