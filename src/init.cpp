 ...
 * between frend, and fren-qt and non-server tools.
*/
...
void PrepareShutdown()
{
    ...
    FlushWallets();
#endif
    GenerateFrens(false, 0, GetParams());
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), FREN_CONF_FILENAME));
    if (mode == HMM_FREND)
    {
#if HAVE_DECL_DAEMON
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), FREN_PID_FILENAME));
#endif
    ...
    strUsage += HelpMessageOpt("-blockmaxweight=<n>", strprintf(_("Set maximum BIP141 block weight (default: %d)"), MAX_BLOCK_WEIGHT - 12000));
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/Apushii/Frencoin>";
    const std::string URL_WEBSITE = "<https://frencoin.org>";
...
/** Sanity checks
 *  Ensure that Fren is running in a usable environment with all
...
static bool LockDataDirectory(bool probeOnly)
{
    ...
    // Make sure only a single Fren process is using the data directory.
...
bool AppInitMain(boost::thread_group& threadGroup, CScheduler& scheduler)
{
    ...
    LogPrintf("Using config file %s\n", GetConfigFile(gArgs.GetArg("-conf", FREN_CONF_FILENAME)).string());
    ...
    GenerateFrens(fGenerate, gArgs.GetArg("-genproclimit", DEFAULT_GENERATE_THREADS), chainparams);
