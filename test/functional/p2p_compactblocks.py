...
# TestNode: A peer we use to send messages to frend, and store responses.
...
class CompactBlocksTest(FrenTestFramework):
    ...
    # This test actually causes frend to (reasonably!) disconnect us, so do this last.
    ...
    # Test that frend requests compact blocks when we announce new blocks
    ...
    def test_incorrect_blocktxn_response(self, node, test_node, version):
        ...
        # Note that it's possible for frend to be smart enough to know we're
    ...
    def test_getblocktxn_handler(node, test_node, version):
        # frend will not send blocktxn responses for blocks whose height is
