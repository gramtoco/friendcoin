...
The Fren Core daemon can be configured to act as a trusted "border
router", implementing the fren wire protocol and relay, making
...
The ZeroMQ feature in Fren Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building frend:
...
For instance:
    ...
    $ frend -zmqpubhashtx=tcp://127.0.0.1:24206 \
               -zmqpubrawtx=ipc:///tmp/frend.tx.raw
...
These options can also be provided in fren.conf.
...
From the perspective of frend, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into frend directly. Furthermore, no information is
...
using. Frend appends an up-counting sequence number to each
