...
The Pejecoin Core daemon can be configured to act as a trusted "border
router", implementing the pejecoin wire protocol and relay, making
...
The ZeroMQ feature in Pejecoin Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building pejecoind:
...
For instance:
    ...
    $ pejecoind -zmqpubhashtx=tcp://127.0.0.1:28766 \
               -zmqpubrawtx=ipc:///tmp/pejecoind.tx.raw
...
These options can also be provided in pejecoin.conf.
...
From the perspective of pejecoind, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into pejecoind directly. Furthermore, no information is
...
using. Pejecoincoind appends an up-counting sequence number to each
