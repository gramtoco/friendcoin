...
The Frencoin Doxygen documentation can be generated by running `make docs` from the top-level 
...
Run with the -testnet option to run with "play frens" on the test network, if you
...
Fren Core is a multithreaded application, and deadlocks or other multithreading bugs
...
- ThreadSocketHandler : Sends/Receives data from peers on port 4207.
...
- ThreadRPCServer : Remote procedure call handler, listens on port 4206 for connections and services them.

- FrenMiner : Generates frens (if wallet is enabled).
...
However, in open source software such as Fren Core, where everyone uses
...
pay attention to for reviewers of Fren Core code.
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: Fren Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against Frencoin so that they can be integrated
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:fren/fren.git
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `fren-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `fren-cli` and the GUI debug console use this table to determine how to
